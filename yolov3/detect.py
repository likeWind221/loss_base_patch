...
def detect(model,img):
    ...
    for i in range(len(pred[0])):
        left = max(int(pred[0][i][0].item()), 0)
        up = max(int(pred[0][i][1].item()), 0)
        right = min(int(pred[0][i][2].item()), inputsize[0])
        below = min(int(pred[0][i][3].item()), inputsize[1])
        confidence = pred[0][i][4].clone().detach()
        results.append({
            'bbox': (up, below, left, right),
            'confidence': confidence,
        })
    return results

def detect_single_target(model, img):
    if len(img.shape) == 3:
        img = img[None]  # expand for batch dim
    inputsize = [416,416]
    img = nn.functional.interpolate(img, (inputsize[0], inputsize[1]), mode='bilinear', align_corners=False)
...
# 求损失
def detect_train1(model, img, target_class_idx=0):
    if len(img.shape) == 3:
        img = img[None]
        
    inputsize = [416, 416]
    img_ = nn.functional.interpolate(img, inputsize, mode='bilinear', align_corners=False).to(img.device)
    
    # 获取模型输出
    preds = model(img_)
    
    # --- 修复核心：自动解包逻辑 ---
    # 如果返回的是 [[T1, T2, T3]] 这种嵌套列表，先剥一层
    if isinstance(preds, (list, tuple)) and len(preds) > 0:
        if isinstance(preds[0], (list, tuple)):
             preds = preds[0]
    
    # 如果 output 只有单个 Tensor 而不是 List，则封装成 List
    ...
    for pred in preds:
        # 此时 pred 必须是 Tensor
        # 如果还是 List，说明嵌套层级非常深或者结构很特殊，打印出来看看
        if not isinstance(pred, torch.Tensor):
            continue 
            
        # pred shape: [Batch, Anchors, Grid, Grid, 85] 
        # 统一展平为 [Batch, N, 85]
        pred_flat = pred.view(pred.shape[0], -1, pred.shape[-1])
        
        # 1. Objectness
        obj_conf = pred_flat[..., 4]
        
        # 2. Class Probability (Person)
        cls_conf = pred_flat[..., 5 + target_class_idx]
